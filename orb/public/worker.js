...
var matches, homo3x3, match_mask, num_matches;
...
img_u8_smooth = new jsfeat.matrix_t(320, 240, jsfeat.U8_t | jsfeat.C1_t);
...
onmessage = function(event) {
	...
	if(event.data.input == 0){
		...
		jsfeat.imgproc.gaussian_blur(img_u8, img_u8_smooth, 3|0);
		stat.stop("gauss blur");
		
		jsfeat.yape06.laplacian_threshold = 0|0;
		jsfeat.yape06.min_eigen_value_threshold = 0|0;
		...
		render_corners(screen_corners, num_corners, data_u32, 320);
		if(pattern_preview) {
			render_mono_image(pattern_preview.data, data_u32, pattern_preview.cols, pattern_preview.rows, 320);
			stat.start("matching");
		...
		render_matches(matches,num_matches,data_u32,320);
		stat.stop("total");
		// Sent processed imagedata back
		var data;
		if(good_matches > 8) data = {dstData: imageData, frame: event.data.frame, fps: stat.log(), homo3x3: homo3x3};
		else data = {dstData: imageData, frame: event.data.frame, fps: stat.log()};
		postMessage(data, [data.dstData.data.buffer]);
		
	} else if(event.data.input == 1){
		var imageData2 = event.data.imageData;
		train(imageData2);
	} else {
...
function train (imageData2) {
	...
	jsfeat.imgproc.grayscale(imageData2.data, 320, 240, img_u8);
